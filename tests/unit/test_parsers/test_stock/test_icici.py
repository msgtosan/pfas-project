...
class TestICICIDirectParser:
    ...
    def test_get_or_create_broker_new(self, db_connection, sample_user):
        ...
        broker_id = parser._get_or_create_broker_via_service(txn_service, "ICICI Direct", user_id=sample_user["id"])
        assert broker_id > 0
    ...
    def test_get_or_create_broker_existing(self, db_connection, sample_user):
        ...
        # Create first time
        broker_id1 = parser._get_or_create_broker_via_service(txn_service, "ICICI Direct", user_id=sample_user["id"])

        # Get same broker again
        broker_id2 = parser._get_or_create_broker_via_service(txn_service, "ICICI Direct", user_id=sample_user["id"])
