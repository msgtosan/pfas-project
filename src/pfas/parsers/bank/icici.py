...
Parses ICICI Bank PDF statements with transaction details.
"""
...
class ICICIParser(BankStatementParser):
    ...
    PERIOD_PATTERN = r"Statement\s+Period[:\s]+(\d{2}-\w{3}-\d{4})\s+to\s+(\d{2}-\w{3}-\d{4})"

    # Transaction line pattern - flexible to handle various formats
    # Matches: Date | Value Date | Description | Debit | Credit | Balance
    ...
    def _parse_content(self, content: dict, source_file: str) -> ParseResult:
        ...
        period_match = re.search(self.PERIOD_PATTERN, text, re.IGNORECASE)
        if period_match:
            try:
                result.statement_period_start = datetime.strptime(
                    period_match.group(1), "%d-%b-%Y"
                ).date()
                result.statement_period_end = datetime.strptime(
                    period_match.group(2), "%d-%b-%Y"
                ).date()
            except ValueError:
                result.add_warning("Could not parse statement period dates")
    ...
    def _parse_transactions_from_tables(self, tables: List[List]) -> List[BankTransaction]:
        ...
        for table in tables:
            ...
            for i in range(header_idx + 1, len(table)):
                ...
                try:
                    # Expected columns: Date | Value Date | Description | Debit | Credit | Balance
                    txn_date = self._parse_date(row[0])
                    value_date = self._parse_date(row[1])
                    description = str(row[2] or "").strip()
                    debit = self._parse_amount(row[3])
                    credit = self._parse_amount(row[4])
                    balance = self._parse_amount(row[5])
